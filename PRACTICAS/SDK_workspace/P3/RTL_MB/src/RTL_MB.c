...
#define DATA_GPIO_ID			XPAR_DATA_GPIO_DEVICE_ID
#define V_Rdy_GPIO_ID   		XPAR_VALID_RDY_GPIO_DEVICE_ID
#define LED_SW_GPIO_ID	    	XPAR_LED_SW_GPIO_DEVICE_ID
#define INPUT					0xFF
#define OUTPUT					0x00
...
/************************** Variable Definitions *****************************/
 ...
 XGpio Data_Gpio;   /* The Instance of the GPIO Driver */
 XGpio Led_Sw_Gpio; /* The Instance of the GPIO Driver */
 XGpio V_Rdy_Gpio;  /* The Instance of the GPIO Driver */
...
int main()
{
	...
	int Status=0;
	u32 DDR_Data = 0;
	u32 DDR_Addr = DDR_BASEADDR;

	/* Initialize the GPIO driver */
	Status = XGpio_Initialize(&Data_Gpio, DATA_GPIO_ID);
	if (Status != XST_SUCCESS) {
		xil_printf("Gpio Initialization Failed\r\n");
		return XST_FAILURE;
	}
	Status = XGpio_Initialize(&Led_Sw_Gpio, LED_SW_GPIO_ID);
	if (Status != XST_SUCCESS) {
		xil_printf("Gpio Initialization Failed\r\n");
		return XST_FAILURE;
	}
	Status = XGpio_Initialize(&V_Rdy_Gpio, V_Rdy_GPIO_ID);
		if (Status != XST_SUCCESS) {
			xil_printf("Gpio Initialization Failed\r\n");
			return XST_FAILURE;
		}
	...
	XGpio_SetDataDirection(&Led_Sw_Gpio, 1, OUTPUT);
	XGpio_SetDataDirection(&Led_Sw_Gpio, 2, INPUT);
	XGpio_SetDataDirection(&Data_Gpio, 1, INPUT);
	XGpio_SetDataDirection(&Data_Gpio, 2, INPUT);
	XGpio_SetDataDirection(&V_Rdy_Gpio, 1, INPUT);
	XGpio_SetDataDirection(&V_Rdy_Gpio, 2, INPUT);
	...
	while(1){
		...
		XGpio_DiscreteWrite(&V_Rdy_Gpio, 2, 0);

      //Print out the current contents of the buffer
      xil_printf("\n\r");
      xil_printf("Options ---------------------\n\r");
      xil_printf("a - Write the SW value in the DDR address\n\r");
      xil_printf("b - Operate with the value stored in the address\n\r");
      xil_printf("\n\r");
      ...
      if (*CommandPtr == 'a'){
    	  	DDR_Data  = XGpio_DiscreteRead(&Led_Sw_Gpio, 2);			//Read board sw value
			writte_ddr((u32*)DDR_Addr, DDR_Data);						//Write in the dddr

			xil_printf("SW Value :   %08X\n\r", DDR_Data);
			xil_printf("Wrote Data : %08X\n\r", DDR_Data);
			xil_printf("Addr : %08X\n\r", DDR_Addr);

      } else if (*CommandPtr == 'b'){
    	  DDR_Data = read_ddr((u32*)DDR_Addr);							//Read ddr
    	  XGpio_DiscreteWrite(&Data_Gpio, 1, DDR_Data);					//write reg

    	  if (DDR_Data == read_ddr((u32*)DDR_Addr)){
    		  XGpio_DiscreteWrite(&V_Rdy_Gpio, 1, 1);					//DDR valid
    	  }

    	  u32 LED_rtl_Value = XGpio_DiscreteRead(&Data_Gpio, 2);		//Read rtl led value
    	  XGpio_DiscreteWrite(&Led_Sw_Gpio, 1, LED_rtl_Value);	//Write board led reg

    	  xil_printf("Addr : %08X\n\r", DDR_Addr);
    	  xil_printf("DDR2 Data : %08X\n\r", DDR_Data);
    	  xil_printf("LED Value : %08X\n\r", LED_rtl_Value);
...
void writte_ddr(u32 *Addr, u32 Data)
{
	...
	/*xil_printf("\n\r");
	xil_printf("Actual Addr : %08X\n\r", Addr);
	xil_printf("Actual Data : %08X\n\r", Data);
	xil_printf("\n\r");*/
