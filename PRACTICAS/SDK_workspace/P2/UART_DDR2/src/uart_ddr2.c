...
#define DDR_HIGHADDR		    0x87ffffff
...
int main()
{
	...
	while(i==0){
      ...
      xil_printf("\n\r");
      xil_printf("Options ---------------------\n\r");
      xil_printf("a - Read the Addr: %08X; and show its ASCII value in up on the LEDs\n\r", Addr);
      xil_printf("b - Press a key to write its ASCII value in the Addr: %08X\n\r", Addr);
      ...
      if (*CommandPtr == 'a'){
    	  ...
    	  //while (ReceiveBufferPtr <= CommandPtr){};
    	  //Addr = Addr - 1;
    	  LED_Value = read_ddr((u32*)Addr);
    	  Status = GpioOutputExample(LED_Value);
      } else if (*CommandPtr == 'b'){
    	  CommandPtr++;
    	  xil_printf("Enter a key to write its ASCII value in the DDR2\n\r");
    	  //xil_printf("Actual CommandPtr : %08X\n\r", *CommandPtr);
    	  while (ReceiveBufferPtr <= CommandPtr){};
    	  LED_Value = *CommandPtr;
    	  writte_ddr((u32*)Addr, LED_Value);
